...
object GzippedStreet{
    def apply(street: Street): GzippedStreet = GzippedStreet( street.lineString, Street.gzip(street))
}

case class GzippedStreet(lineString: LineString, street: Array[Byte]) extends Geometry(lineString.getFactory) {
    ...
    def toStreet = Street.ungzip(street, lineString)
