...
class ShapeLoader() extends Loader[OSMStreet]{
  ...
  override def loadIndex(sources: String*): GeometryList[OSMStreet] = {
    ...
    val lines: Iterator[OSMStreet] = sources.foldLeft(Seq.empty[OSMStreet].toIterator)( (acc, source) => acc ++ loadFile(source).map(e => {
      ...
      val streetType: String = if(fields(4) != null) fields(4).toString else ""
      ...
      val oneway: Boolean = if(fields(5) != 0) true else false
      val speedlimit: Int = if(fields(6) != 0) fields(6).toString.toInt else 0
      //val toSpeed: Integer = fields(6).toInt
      i += 1

      OSMStreet(lr, street, speedlimit, oneway, st)
